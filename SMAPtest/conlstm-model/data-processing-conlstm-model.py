...
def get_data_and_lable_for_ConLstm(SMAPdata,seq_length,lead_time):
    ...
    print('sss', data.shape, label.shape)
    return data, label
...
if __name__ == '__main__':
    p = ArgParser()
    p.add_argument('--datadir', type=str, default='../../../SMAP/smapMerge_52_65/', help='Path to data')
    p.add_argument('--seq_length', type=int, default=3, help='input timesteps for ConLSTM model')
    p.add_argument('--lead_time', type=int, default=7, help='Forecast lead time')
    args = p.parse_args()
