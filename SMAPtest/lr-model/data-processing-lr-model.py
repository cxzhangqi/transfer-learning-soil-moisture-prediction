...
def main(datadir,seq_length,lead_time):
    ...
    if not os.path.exists(dir):
        os.mkdir(dir)
    np.save('./data-lr-model/features_train.npy', features_train)
    np.save('./data-lr-model/features_test.npy', features_test)
    np.save('./data-lr-model/label_train.npy', label_train)
    np.save('./data-lr-model/label_test.npy', label_test)
    np.save('./data-lr-model/scalar_set.npy', scalar_set)
    print('finished saving  data for lr model')


if __name__ == '__main__':
    p = ArgParser()
    p.add_argument('--datadir', type=str, default='../../../SMAP/smapchina52_65/', help='Path to data')
    p.add_argument('--seq_length', type=int, default=3, help='input timesteps for lstm model')
    p.add_argument('--lead_time', type=int, default=30, help='Forecast lead time')
    args = p.parse_args()
