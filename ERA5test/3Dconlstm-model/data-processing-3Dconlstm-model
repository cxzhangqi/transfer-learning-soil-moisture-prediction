...
from ERA5datacode import generatedataERA5


def get_data_and_lable_for_ConLstm(SMAPdata,seq_length,lead_time):
    rawdata = SMAPdata
    data = []
    label = []
    for i in range(0, rawdata.shape[0] - seq_length-lead_time):
        tmp_data = rawdata[i:i + seq_length]
        tmp_label = rawdata[i + seq_length:i + seq_length+lead_time,rawdata.shape[1]-1,:]
...
def main(datadir,seq_length,lead_time):
    ...
    SMAPdata = generatedataERA5(datadir)
    print('finished generating SMAP data')
    # TODO: split datasets into training dataset and testing dataset
    ####training dataset
    data_train = SMAPdata[0:int(SMAPdata.shape[0]) - 365 - seq_length - lead_time]
    ####testing dataset
    ...
    features_train, label_train = get_data_and_lable_for_ConLstm(data_train, seq_length, lead_time)
    ...
    np.save('./data-ConLSTM-model/data_train.npy', data_train)

    print('finished saving  data for ConLSTM model')


if __name__ == '__main__':
    p = ArgParser()
    p.add_argument('--datadir', type=str, default='../../../era5_xinjiang_50_65/', help='Path to data')
